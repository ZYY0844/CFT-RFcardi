...
class Trainer(nn.Module):
    ...
    def _prepare_model(self, weighting, architecture, encoder_class, decoders):
        ...
        # Freeze the parameters of the decoder named 'ECG_shape'
        # for param in decoders['ECG_shape'].parameters():
        #     param.requires_grad = False

        # for param in decoders['Anchor'].parameters():
        #     param.requires_grad = False
        # if parameter in backbone
        # for param in self.model.encoder.parameters():
        #     param.requires_grad = False
        # Filter out the parameters that require gradients
        # self.optim_param['params'] = filter(lambda p: p.requires_grad, self.model.parameters())


        if self.load_path is not None:
            if os.path.isdir(self.load_path):
                self.load_path = os.path.join(self.load_path, 'best.pt')
            # saved_state_dict = torch.load(self.load_path)
            # # only load the model for encoder.backbone
            # for key in saved_state_dict.keys():
            #     if 'encoder.backbone' in key:
            #         saved_state_dict[key] = saved_state_dict[key].to(self.device)
