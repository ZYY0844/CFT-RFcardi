...
def cross_entropy_loss_ppi(ecg_rcon, ecg_gts):
    ...
    # count how many -10 are there in each batch of ecg_gts
...
def sparseness(x):
    length = torch.tensor(x.size()[-1]).to(x.device)
    return torch.mean((torch.sum(torch.abs(x), axis=2)/torch.sqrt(torch.sum(x**2, axis=2))-1)/(torch.sqrt(length)-1))
...
class anchorMetric(AbsMetric):
    ...
    def update_fun(self, pred, gt):
        gt = torch.clone(gt).detach()
        gt = normal_ecg_torch_01(gt)
        pred_norm = normal_ecg_torch_01(torch.clone(pred).detach())
        # mse = criterion_mse(pred_norm, gt)
        mse = criterion_mse(pred_norm, gt)
        spars = sparseness(pred_norm) ### sparsity
        # spars = torch.mean(torch.abs(pred))
        self.mse.append(mse.item())
        self.sparsity.append(spars.item())
        self.bs.append(pred.size()[0])
    def score_fun(self):
        records = np.array(self.mse)
        batch_size = np.array(self.bs)
        mse = (records*batch_size).sum()/(sum(batch_size))
        records = np.array(self.sparsity)
        sparsity = (records*batch_size).sum()/(sum(batch_size))
        return [mse, sparsity]
    def reinit(self):   
        self.mse = []
        self.sparsity = []
