...
from Projects.radarODE_transfer.nets.ODE_solver import ECGParameterEstimator, ode1_solver, scale_output
...
class PPI_decoder(nn.Module):
    def __init__(self, output_dim, dim=512):
        super().__init__()
        self.conv = nn.Sequential(
            ConvBlock(dim, dim//4, kernel_size=7, stride=1, padding=1),
            ConvBlock(dim//4, dim//8, kernel_size=7, stride=1, padding=1),
            ConvBlock(dim//8, dim//16, kernel_size=7, stride=1, padding=1),
            ConvBlock(dim//16, dim//32, kernel_size=6, stride=1, padding=1),
        )
        self.transconv = nn.Sequential(
            convTransBlock(dim//32, dim//32, kernel_size=5, stride=2, padding=2, output_padding=1),
            convTransBlock(dim//32, dim//16, kernel_size=5, stride=2, padding=2, output_padding=1),
            convTransBlock(dim//16, dim//16, kernel_size=5, stride=1, padding=2, output_padding=0),
