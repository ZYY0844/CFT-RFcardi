...
class CNNLSTMDecoder(nn.Module):
    def __init__(self, dim=64):
        ...
        #     ConvBlock(dim, dim//2, kernel_size=7, stride=2, padding=1),
        #     ConvBlock(dim//2, dim//4, kernel_size=7, stride=2, padding=1),
        #     ConvBlock(dim//4, dim//8, kernel_size=7, stride=1, padding=1),
        #     ConvBlock(dim//8, dim//8, kernel_size=5, stride=1, padding=1),
        # )
        self.conv = nn.Sequential(
            ConvBlock(dim, dim//2, kernel_size=7, stride=2, padding=1),
            ConvBlock(dim//2, dim//4, kernel_size=7, stride=1, padding=1),
            ConvBlock(dim//4, dim//8, kernel_size=7, stride=1, padding=1),
            # ConvBlock(dim//8, dim//8, kernel_size=5, stride=1, padding=1),
        )

        self.out_conv = nn.Sequential(
            nn.Conv1d(dim//8, dim//16, kernel_size=7, stride=1, padding=2),
            nn.Conv1d(dim//16, dim//32, kernel_size=5, stride=1, padding=3),
            nn.Conv1d(dim//32, 1, kernel_size=2, stride=1, padding=0),
        )
